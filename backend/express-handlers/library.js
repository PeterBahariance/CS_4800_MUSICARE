...
async function handleSaveItem(req, res) {
    const { userId, itemId, itemType, context, playlistData } = req.body || {};
    ...
    try {
        if (itemType === 'playlist') {
            const saved = await savePlaylistToLibrary(userId, itemId, context, playlistData);
            return res.status(200).json({
 ...
 */
async function savePlaylistToLibrary(userId, playlistId, context, playlistData) {
    ...
    if (!existingPlaylist) {
        if (!playlistData || !Array.isArray(playlistData.tracks) || playlistData.tracks.length === 0) {
            throw new Error('Playlist data required to save this playlist');
        }

        await createPlaylistFromData(playlistId, playlistData);
    }
...
async function createPlaylistFromData(playlistId, playlistData) {
    const tracks = Array.isArray(playlistData.tracks) ? playlistData.tracks : [];

    await prisma.playlist.create({
        data: {
            id: playlistId,
            title: playlistData.title || 'Saved Playlist',
            description: playlistData.description || null,
            mood: playlistData.mood || 'relaxation',
            coverImage: playlistData.coverImage || null,
            verified: false,
            category: playlistData.category || null,
            categoryKey: playlistData.categoryKey || null,
            playlistSongs: {
                create: tracks.map((track, index) => ({
                    position: typeof track.position === 'number' ? track.position : index,
                    song: buildSongConnectOrCreate(track, `${playlistId}-track-${index}`)
                }))
            }
        }
    });
...
function buildSongConnectOrCreate(track, fallbackId) {
    const jamendoId = track?.jamendoId || track?.id || null;
    const uniqueWhere = jamendoId
        ? { jamendoId }
        : { id: track?.id || fallbackId || randomUUID() };

    const createData = {
        id: jamendoId || track?.id || fallbackId || randomUUID(),
